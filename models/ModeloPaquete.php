...
class ModeloPaquete {
  ...
  static public function mdlRegistro($table, $data) {
    ...
    $stmt = Connection::connect()->prepare("INSERT INTO $table( titulo, texto, img, precio ) VALUES ( :titulo, :texto, :img, :precio )");
  ...
  static public function mdlMostrarTresRes( $table ){
    $stmt = Connection::connect()->prepare("SELECT * FROM $table ORDER by id DESC LIMIT 3");
    $stmt -> execute();
    return $stmt -> fetchAll();
  }
    ...
    static public function mdlEditarRegistro($table, $data){
      ...
      $stmt = Connection::connect()->prepare("UPDATE $table SET titulo  = :titulo, texto = :texto, img = :img, precio = :precio WHERE id = :id");
